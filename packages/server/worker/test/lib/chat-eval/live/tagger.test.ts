...
function toolCall({ toolName, input = {}, actionName, connectorName, outputText = '', status = PersistedToolCallStatus.COMPLETED }: {
    toolName: string
    ...
    const fullInput = { ...input, ...(actionName ? { actionName } : {}), ...(connectorName ? { connectorName } : {}) }
...
describe('liveTagger.tag', () => {
    it('scores a clean discovery→execute success', () => {
        const uiMessages = [assistant([
            ...
            toolCall({ toolName: 'ap_get_piece_props', input: { connectorName: 'gmail', actionOrTriggerName: 'send_email' } }),
    ...
    it('counts schema re-fetches, bad-arg rejections, and the breaker hit', () => {
        const uiMessages = [assistant([
            toolCall({ toolName: 'ap_get_piece_props', input: { connectorName: 'slack', actionOrTriggerName: 'send_message' } }),
    ...
    it('grades native handling: code task via ap_run_code, HTTP task via the http piece', () => {
        ...
        const http = liveTagger.tag({
            ...
            uiMessages: [assistant([toolCall({ toolName: 'ap_execute_action', connectorName: '@wippa/connector-http', outputText: '✅ ok' })])],
        })
    ...
    it('treats an ACTION_RECEIPT success as goal met even if no execute text is present', () => {
        const uiMessages = [assistant([
            ...
            { type: PersistedChatPartType.ACTION_RECEIPT, toolCallId: 'r1', actionDisplayName: 'Send Message', connectorName: 'slack', status: 'success', timestamp: 't' },
        ])]
        const score = liveTagger.tag({ scenario: scenario(), uiMessages })
        expect(score.executeSucceeded).toBe(true)
        expect(score.gaveUp).toBe(false)
