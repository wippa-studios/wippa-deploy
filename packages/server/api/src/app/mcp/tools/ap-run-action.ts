...
const runActionInput = z.object({
    connectorName: z.string().describe('Piece name, e.g. "slack" or "@wippa/connector-slack". Use ap_research_pieces to discover.'),
...
export const apRunActionTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { connectorName, actionName, input, connectionExternalId } = runActionInput.parse(args)
