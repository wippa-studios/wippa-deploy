...
async function updateTriggerEvent(
    ...
    for (const triggerEvent of triggerEvents) {
        if (triggerEvent.source) {
            if (revert) {
                triggerEvent.source = `@wippa/piece-${triggerEvent.source}`
            }
            else {
...
function updateStep(step: Step | undefined, revert: boolean): boolean {
    ...
    while (step) {
        if (step.type === PIECE_TYPE || step.type === PIECE_TRIGGER_TYPE) {
            (step.settings as any).pieceName = getPackageNameForPiece(
                (step.settings as any).pieceName,
                revert,
            )!
            update = true
        }
...
const getPackageNameForPiece = (
    ...
    if (revert) {
        return pieceName.replace('@wippa/piece-', '')
    }
    return `@wippa/piece-${pieceName}`
}
