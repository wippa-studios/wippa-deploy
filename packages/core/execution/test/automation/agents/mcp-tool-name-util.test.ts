...
describe('createPieceToolName', () => {
    it('strips @scope/piece- prefix', () => {
        expect(createPieceToolName('@wippa/connector-slack', 'send_message')).toBe('slack-send_message_pqyv3q_mcp')
    })
    ...
    it('normalizes the combined name correctly', () => {
        expect(createPieceToolName('@wippa/connector-google-sheets', 'insert_row')).toBe('google-sheets-insert_row_q388b6_mcp')
    })
})
