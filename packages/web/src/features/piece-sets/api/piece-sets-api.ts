...
export const pieceSetsApi = {
  ...
  get(id: string) {
    return api.get<ConnectorSet>(`/v1/piece-sets/${id}`);
  },
  create(request: CreatePieceSetRequestBody) {
    return api.post<ConnectorSet>('/v1/piece-sets', request);
  },
  update(id: string, request: UpdatePieceSetRequestBody) {
    return api.post<ConnectorSet>(`/v1/piece-sets/${id}`, request);
  },
  delete(id: string) {
    return api.delete<void>(`/v1/piece-sets/${id}`);
  },
  duplicate(id: string, request: DuplicatePieceSetRequestBody) {
    return api.post<ConnectorSet>(`/v1/piece-sets/${id}/duplicate`, request);
  },
