...
Add a `refresh` field to cache the token server-side. Wippa renews it automatically up to 15 minutes before expiry (clamped to half the token lifetime for short-lived tokens).
