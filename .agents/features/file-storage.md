...
- **Piece API:** `FilesService.write({ fileName, data: Buffer | Readable })` (`@wippa/pieces-framework` ≥ 0.34.0). A `Buffer` keeps the exact existing behaviour; a `Readable` streams.
